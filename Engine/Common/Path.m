classdef Path
    ...
    methods (Static)
        function AddAllSubfolders(strStartDir, caStrsIgnoreDirs, strXorIntAddpathMode, blnRemDirs, blnDebug)
            ...
            caAddRemDirs = {};
            [caAddRemDirs] = Path.RecursiveAddPath(caAddRemDirs, strStartDir, caStrsIgnoreDirs, strXorIntAddpathMode, blnRemDirs,blnDebug);
        ...
        function AddToPath(fullpath)
            % Simply add a path to the system path.
            assert(isstring(fullpath),"Expecting a valid path string.");
            addpath(fullpath);
        end
        function [flag] = IsOnPath(fullpath)
            % Test if a path is on the current system path.
            assert(isstring(fullpath),"Expecting a valid path string.");
            % Simply get all paths and check if its a member
            flag = any(ismember(fullpath,regexp(path,pathsep,'Split')));
        end
    end

    methods (Static,Access = private)
        function [caAddRemDirs] = RecursiveAddPath(caAddRemDirs, strStartDir, caStrsIgnoreDirs, strXorIntAddpathMode, blnRemDirs, blnDebug)
            ...
            for intDirIndex = 1 : length(saSubDirs)
                ...
                if ~(blnIgnoreDir || blnDirBegins)
                    ...
                    [caAddRemDirs] = Path.RecursiveAddPath(caAddRemDirs, strThisStartDir, caStrsIgnoreDirs, strXorIntAddpathMode, blnRemDirs, blnDebug);
                end
